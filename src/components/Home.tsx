...
const Home = () => {
  ...
  const handleItemSelected = (item: MediaItem) => {
  ...
  return (
    <>
      <h2>My Media</h2>
      {selectedItem && <SingleView mediaItem={selectedItem} />}
      <table>
        <thead>
          <tr>
            <th>Thumbnail</th>
        ...
        <tbody>
          ...
          {mediaArray.map((item) => (
            <MediaRow key={item.media_id} mediaItem={item} onItemSelected={() => handleItemSelected(item)} />
          ))}
        </tbody>
      </table>
    </>
